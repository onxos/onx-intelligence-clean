  ...
  if (mode !== "mysql") {
    ...
    if (mode === "pg") {
      ...
      try {
        ...
        // The transaction already recorded the failure loudly (op
        // "iurg-pg.replaceObjects") and rolled back — pg is not corrupted.
        // Bump the legacy HT-09 counter and stay fail-open on the mirror.
        counters.pgErrors += 1;
      }
      return;
    }
  ...
  if (mode !== "mysql") {
    ...
    if (mode === "pg") {
      ...
      try {
        ...
        // Transaction already recorded + rolled back (op
        // "iurg-pg.replaceObjectsByPrefix"); bump legacy counter, stay fail-open.
        counters.pgErrors += 1;
      }
      return;
    }
