...
export const ACTION_CATALOG: Record<string, { description: string; params: string[]; costly?: boolean }> = {
  ...
  engine_run: { description: "تشغيل محرك تحليل فوراً: content-intelligence|conversion|creative|crm|executive|influencer|local|paid|referral|reputation|retention|sales|social|veterinary (action=analyze) — geo-intelligence(check/report) — journey-weaver(execute) — kpi-anomalies(detect, kpiId) — kpi-forecast(generate, kpiId) — kpi-insights/kpi-recommendations(generate, kpiId) — market-digital-twin(simulate) — predictive-demand(generate) — seo-intelligence(audit/track_keyword) — signal-harvesters(run_all/run) — autonomous-campaign-engine(plan) — competitor-intelligence(add_competitor/observe) — anomaly-shield(detect/detect_spend/detect_ctr/detect_cv/detect_cpm/detect_fraud) — intent-predictor(generate) — opportunity-discovery(discover) — content-engine(daily_run)", params: ["engine", "action?", "params?"] },
...
async function executeAction(action: string, params: Record<string, unknown>): Promise<unknown> {
  switch (action) {
    ...
    case "engine_run": {
      if (!params.engine) return { error: "engine required" };
      return bridgeCall("engine-invoke", { engine: params.engine, action: params.action, params: (params.params as Record<string, unknown>) ?? {} });
    }
    case "record_create":
    case "record_update": {
