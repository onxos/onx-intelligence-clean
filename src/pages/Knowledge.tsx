  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-950 text-white p-6">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-indigo-400">ðŸ“š Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ù…Ø¹Ø±ÙØ©</h1>
          <p className="text-gray-400 mt-1">22,500 Ø³Ø¬Ù„ â€¢ 19 Ù…Ø¬Ø§Ù„ â€¢ Ø¨Ø­Ø« Ø¯Ù„Ø§Ù„ÙŠ Ø¨Ù€ GPT-4o</p>
        </div>
        ...
        {statsQ.data && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { label: "Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø³Ø¬Ù„Ø§Øª", value: statsQ.data.totalRecords.toLocaleString(), color: "text-indigo-400" },
              { label: "Ø§Ù„Ù…Ø¬Ø§Ù„Ø§Øª", value: statsQ.data.domains, color: "text-blue-400" },
              { label: "Ø¹Ù…Ù„ÙŠØ§Øª Ø§Ù„Ø¨Ø­Ø«", value: statsQ.data.totalSearches, color: "text-purple-400" },
              { label: "Ù…ØªÙˆØ³Ø· Ø§Ù„Ø«Ù‚Ø©", value: statsQ.data.avgConfidence, color: "text-green-400" },
            ].map((c) => (
        ...
        <div className="bg-gray-900 border border-indigo-700 rounded-xl p-5 mb-6">
          <h2 className="text-lg font-bold mb-3 text-indigo-300">ðŸ” Ø§Ù„Ø¨Ø­Ø« ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ù…Ø¹Ø±ÙØ©</h2>
          <div className="flex gap-2">
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
              placeholder="Ø§Ø¨Ø­Ø« ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ù…Ø¹Ø±ÙØ©..."
          ...
          {semanticMut.data && (
            <div className="mt-4">
              <div className="text-sm text-gray-400 mb-2">
                Ø§Ù„Ù…Ø¬Ø§Ù„Ø§Øª: {semanticMut.data.semanticDomains.join(', ')} â€¢ {semanticMut.data.returned} Ù†ØªÙŠØ¬Ø©
              </div>
                ...
                {semanticMut.data.results.map((r) => (
                  <div key={r.id} className="bg-gray-800 rounded-lg p-3 border border-gray-700">
                    <div className="flex justify-between">
                      ...
                      <span className="text-xs text-gray-500">{r.domain} â€¢ {r.score.toFixed(0)}pts</span>
                    </div>
          ...
          {semanticMut.isPending && <div className="text-center text-indigo-400 mt-4">ðŸ§  ÙŠØ­Ù„Ù„ Ø§Ù„Ø§Ø³ØªØ¹Ù„Ø§Ù… Ø¯Ù„Ø§Ù„ÙŠØ§Ù‹...</div>}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {domainsQ.data && (
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-5">
              <h2 className="text-lg font-bold mb-3 text-blue-300">ðŸ—‚ï¸ Ø§Ù„Ù…Ø¬Ø§Ù„Ø§Øª (19)</h2>
              <div className="grid grid-cols-2 gap-2">
                {domainsQ.data.domains.slice(0, 12).map((d) => (
                  <div key={d.id} className="bg-gray-800 rounded-lg p-2 text-center">
                    ...
                    <div className="text-xs text-gray-500">{d.recordCount.toLocaleString()} Ø³Ø¬Ù„</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {trendingQ.data && (
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-5">
              <h2 className="text-lg font-bold mb-3 text-purple-300">ðŸ”¥ Ø§Ù„Ø£ÙƒØ«Ø± ÙˆØµÙˆÙ„Ø§Ù‹</h2>
              <div className="space-y-2">
