...
- 2026-06-28 AV6-01 capital allocation merged: PR #1 merged to main via squash on commit 6c850e1a1d2d179dd6eed5f24f3212677fde1e97 (implementation commit 584dd69873643aba57db5687e8d033943f349e7c), Render success https://github.com/onxos/onx-intelligence-clean/actions/runs/28316881675, production /commit matches main, production /health ok, smoke PASS, production capital allocation/policy lifecycle proof PASS, production capital reports/history/audit proof PASS, production OpenAPI capital DTOs and endpoints proof PASS, CI run https://github.com/onxos/onx-intelligence-clean/actions/runs/28316881670 failed at lint step and requires follow-up.
- 2026-06-28 AV6-01G governance closure: AV6-01 status CLOSED and Capital Allocation status COMPLETE on commit ff3fd7fb37fa436c5d46816e589c2066ef82b5d5 (`fix(ci): restore green lint pipeline`), CI success https://github.com/onxos/onx-intelligence-clean/actions/runs/28318069935, Render success https://github.com/onxos/onx-intelligence-clean/actions/runs/28318069946, production `/commit` synchronized to ff3fd7fb37fa436c5d46816e589c2066ef82b5d5, production `/health` status ok, smoke PASS, regression PASS (`npm run lint`, `npm run build`, `npm test`, `npm run test:e2e`), remaining Atlas V6 scope: Founder Intent Compiler (Atlas V6 only), USFIP (Future Research), Proof Stress Architecture (Future Research).
- 2026-06-29 MO-032 capital runtime repair publication complete: Prisma-only capital runtime enforced on commit c3a50a587027506b40816e010ef42f8a3d487e3d (`fix(capital): enforce Prisma-only runtime for ACE lock`), CI success https://github.com/onxos/onx-intelligence-clean/actions/runs/28349763882, Render success https://github.com/onxos/onx-intelligence-clean/actions/runs/28349763890, production `/commit` synchronized to c3a50a587027506b40816e010ef42f8a3d487e3d, production `/health` status ok with database up, smoke PASS, production Capital allocation/policy/reports/history/audit lifecycle proof PASS, production OpenAPI capital endpoints PASS, runtime breach removed (`canUseDatabase` removed, in-memory fallback removed, Prisma-only path enforced).
- 2026-06-30 RWO-01 repository implementation completion executed on `main`: runtime integrity repairs applied (`docker-compose` start command corrected to `node dist/src/main.js`; auth paths now return explicit `503 Database unavailable` when Prisma is offline; Prisma startup now skips connect when `DATABASE_URL` is unset), local CI PASS (`npm run ci`), database integrity PASS (`npm run db:deploy`, `npm run db:seed`, `npx prisma migrate status` against local Postgres), and smoke PASS (`BASE_URL=http://localhost:3000 bash scripts/smoke.sh`).
