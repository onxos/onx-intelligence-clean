...
**تحديث الحالة المقاسة (STE-K-34، عند commit `7d6853a`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أظهر **27 لقطة** إنتاجية (`truth_ledger_read`: 27 snapshots،
...
بعد فالأصل محفوظ بصدق؛ التقليم المقاس يبدأ بعد تجاوز 168. سطح `/commit` المقاس عبر أصل البوابة
يخدم `commit=7d6853aa1c15…` (متقارب مع `7d6853a`). حد المعدل **`POSTGRES_PERSISTED` مقاس حياً**
