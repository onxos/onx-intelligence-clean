...
**تحديث الحالة المقاسة (STE-K-24، عند commit `42d59d4`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أظهر **16 لقطة** إنتاجية (`truth_ledger_read`: 16 snapshots،
...
مفاتيح) وتعرض الآن (STE-K-23) **بطاقة احتفاظ** (keep/oldestRetainedId/شارة حافة genesis-retained مقابل
older-pruned) و**شارة إصرار حد المعدل مقاسة** — واستُبدل التعليق الصلب البائت «per-instance in-memory»
الذي كان يناقض مخزن K-19 المقاس بتعليق يتبع الشارة الحية. كل الأسطح التسعة تُقاس **عبر الرابط الواحد**
`…/intelligence` بنتيجة 9/9 (STE-K-20، و.9). الكوربوس يبقى `disclosure=DEMO` مقاساً (22500 بذرة قالبية،
sha256 `6fc2bed87d86…`) **بانتظار أرشيف REC-06 الأصيل** (19,012 وثيقة) لينقلب REAL **بالقياس لا باليد**.
