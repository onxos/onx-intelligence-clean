...
**تحديث الحالة المقاسة (STE-K-40، قياس قبل الكتابة عند commit `8169151`):** السجل **ينمو ساعياً** من كرون الويب —
...
والإجمالي المستقل يظهر في `onx.selfVerify.truthLedgerSummary.count=36` (`persistence=POSTGRES`).
...
سطح `/health` المقاس عبر أصل البوابة يخدم `commit=816915140b2d…` (متقارب مع `8169151`). حد المعدل **`POSTGRES_PERSISTED` مقاس حياً**
