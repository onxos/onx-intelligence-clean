...
**تحديث الحالة المقاسة (STE-K-39، قياس قبل الكتابة عند commit `1f7b2c9`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أكّد دلالة العدّ: `onx.truthHistory.count` = **حجم نافذة
الاستجابة** لا إجمالي الجدول. القياس المباشر: `limit=20 => count=20` (الافتراضي)، `limit=100 => count=35`،
والإجمالي المستقل يظهر في `onx.selfVerify.truthLedgerSummary.count=35` (`persistence=POSTGRES`).
...
سطح `/health` المقاس عبر أصل البوابة يخدم `commit=1f7b2c9624bb…` (متقارب مع `1f7b2c9`). حد المعدل **`POSTGRES_PERSISTED` مقاس حياً**
