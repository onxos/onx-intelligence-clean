...
**تحديث الحالة المقاسة (STE-K-25، عند commit `f638194`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أظهر **18 لقطة** إنتاجية (`truth_ledger_read`: 18 snapshots،
0 drift-flagged، `persistence=POSTGRES`). **احتفاظ السجل مُفصَح حياً** (STE-K-22): `retention =
{keep:168, oldestRetainedId:1, oldestRetainedIsGenesis:true}` — النافذة (168 = 7 أيام ساعية) لم تُبلَغ
بعد فالأصل محفوظ بصدق؛ التقليم المقاس يبدأ بعد تجاوز 168. حد المعدل **`POSTGRES_PERSISTED` مقاس حياً**
(STE-K-19، و.5). صفحة `/truth` العامة **حية** (HTTP 200، مبنية كلياً من الأسطح الصادقة، صفر تسريب
مفاتيح) وتعرض (STE-K-23) **بطاقة احتفاظ** (keep/oldestRetainedId/شارة حافة genesis-retained مقابل
older-pruned) و**شارة إصرار حد المعدل مقاسة** — واستُبدل التعليق الصلب البائت «per-instance in-memory»
الذي كان يناقض مخزن K-19 المقاس بتعليق يتبع الشارة الحية. و**برهان الرسم مُفصَح** (STE-K-25): الفحص
التاسع `no_key_leak` يُثبت الآن أن الصفحة المخدومة هي قشرة SPA المبنية الحقيقية بعلامات مقاسة
(`id="root"` + `<script type="module" src="/assets/…">`) — البرهان الحي `RENDER_PROVEN=true`. كل الأسطح
التسعة تُقاس **عبر الرابط الواحد** `…/intelligence` بنتيجة 9/9 (STE-K-20، و.9). الكوربوس يبقى
`disclosure=DEMO` مقاساً (22500 بذرة قالبية، sha256 `6fc2bed87d86…`) **بانتظار أرشيف REC-06 الأصيل**
(19,012 وثيقة) لينقلب REAL **بالقياس لا باليد**.
