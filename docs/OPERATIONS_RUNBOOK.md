...
**تحديث الحالة المقاسة (STE-K-21، عند commit `01c18a1`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أظهر **13 لقطة** إنتاجية (`truth_ledger_read`: 13 snapshots،
0 drift-flagged، `persistence=POSTGRES`، أحدث بصمة `bb642469c81d…`). حد المعدل **`POSTGRES_PERSISTED`
مقاس حياً** (STE-K-19، و.5). صفحة `/truth` العامة **حية** (HTTP 200، مبنية كلياً من الأسطح الصادقة، صفر
تسريب مفاتيح). كل الأسطح التسعة تُقاس **عبر الرابط الواحد** `…/intelligence` بنتيجة 9/9 (STE-K-20، و.9).
الكوربوس يبقى `disclosure=DEMO` مقاساً (22500 بذرة قالبية، sha256 `6fc2bed87d86…`) **بانتظار أرشيف
REC-06 الأصيل** (19,012 وثيقة) لينقلب REAL **بالقياس لا باليد**.
