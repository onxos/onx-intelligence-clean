...
2. **أرضيات golden سلك تعثّر لا يُخفَض أبداً**: `api/fixtures/eval-floors.json` =
   ...
   - **تحديث STE-K-33 (مقاس):** مجموعة `GOLDEN_SET` تعمّقت من **49 → 57** حالة
     (تعميق +8، لا تغيير أسطح) مع ثلاث فئات صريحة: (أ) نوايا تشغيلية/طوارئ مع
     رفض صادق على كوربوس DEMO، (ب) استرجاع معرفي مشتق من نطاقات DEMO الفعلية
     (LEGAL/MEDICINE/EDUCATION/FINANCE/ENVIRONMENT/TRANSPORTATION وغيرها)، (ج)
     رفض خارج-المعرفة (طقس/سياسة/طبخ/رياضة). الراتشيت يبقى 1.0×3 ويُقاس على
     العدد الجديد بلا خفض.
...
**تحديث الحالة المقاسة (STE-K-31، عند commit `6e995f7`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أظهر **24 لقطة** إنتاجية (`truth_ledger_read`: 24 snapshots،
