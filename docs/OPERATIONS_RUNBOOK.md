...
**تحديث الحالة المقاسة (STE-K-37، قياس قبل الكتابة عند commit `425cc05`):** السجل **ينمو ساعياً** من كرون الويب —
القياس الحي الأحدث عبر أصل البوابة الرسمية أظهر **20 لقطة** إنتاجية (`truth_ledger_read`: 20 snapshots،
...
بعد فالأصل محفوظ بصدق؛ التقليم المقاس يبدأ بعد تجاوز 168. سطح `/health` المقاس عبر أصل البوابة
يخدم `commit=425cc05d003a…` (متقارب مع `425cc05`). حد المعدل **`POSTGRES_PERSISTED` مقاس حياً**
