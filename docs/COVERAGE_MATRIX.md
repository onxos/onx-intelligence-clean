...
| K4 | K4-GOVERNED-METHODS | ✅ VERIFIED (منفذ ومثبت) | ربط مناهج B2-β فعلياً بدورة الـOrchestrator (B2) — PR #64 مدموج (**merge sha `1aa04e8`**، دمج المنسق الأعلى بعد بوابته السطرية المستقلة على `89cfb5c` — لا دمج ذاتي؛ CI على main بعد الدمج: Codex Guard run 29198183497 + UEP run 29198183515 كلاهما SUCCESS): `TaskSpec.methodId` اختياري لكن fail-closed مرتين: `planMandate` يستدعي `requireMethod` (منهج مجهول يفشل الخطة قبل أي تنفيذ) و`runTask` بعد `independentlyVerify` يشغّل `verifyMethodCompliance` على دليل يُجمَع من **مصدر مستقل** (`registerMethodEvidenceSource` — مصنوعات repo/CI/PR/runtime خارج المنفذ): `methodOutput` المرفق من المنفذ يُسجَّل كغير موثوق ويُتجاهَل (نفس فلسفة `claimedComplete` — لا شهادة ذاتية)؛ لا مصدر مسجّل أو دليل أجوف (`{}`/مجموعات فارغة) → REJECTED. منهجا `code-review` و`test-fixing` الجديدان يفرضان مطابقة per-ref مؤرّخة: كل MERGE يحتاج REVIEW مطابق الـref قبله/معه، كل FIX يحتاج RUN استنساخ قبله + دليل TEST، وغياب التاريخ = رفض. اختبارات: methods-library 60 (منها 7 anti-self-certification) + orchestrator 34 (منها 8 K4) + خطوة CI مخصصة «Governed Methods Enforcement (K4)» بفلاتر `-t` تثبت تشغيلها (سجل خام مؤكد run 29197491507: methods 7 + orchestrator 8). معيار `ac-k4-governed-methods` — رُقّي بعد الدمج بأثر متحقَّق مستقلاً |
| K1 | K1-DEEP-RESEARCH | ✅ VERIFIED (منفذ ومثبت) | حلقة البحث العميق المتكرر — PR #65 مدموج (**merge sha `2a3eeda`**، دمج المنسق الأعلى بعد بوابتين مستقلتين: تدقيق سطري لمنسق v2 على الرأس `68ce8d4` + تشغيل 33/33 في worktree تدقيق منفصل، ثم بوابة المنسق الأعلى): `api/lib/deep-research.ts` قدرة بحثية حتمية بلا مفاتيح/شبكة بخمس حالات صريحة — **plan** (سؤال → قائمة استعلامات فرعية **مغلقة** عند التخطيط، لا توسع runtime؛ سؤال فارغ → `EMPTY_QUESTION` fail-closed) → **collect** (عبر **provider port** قابل للحقن `SourceProvider` **مملوك للخادم**؛ الراوتر لا يقبل fixtures من العميل — `makeUnavailableProvider`/`makeDemoProvider` حتميان، لا شبكة) → **validate** (fail-closed: اكتمال الحقول، اتساق التاريخ مقابل `now`، عتبة موثوقية موثقة؛ المصدر الناقص/الضعيف/المستقبلي يُستبعد ويُعدّ) → **contradict** (**إعادة استخدام B5** `runRealityPipeline`/`detectContradictions` — لا منطق تناقض جديد) → **report** (كل ادعاء مربوط بمعرفات مصادره؛ **الاستشهاد إلزامي**؛ التناقضات غير المحسومة تُعرض ولا تُخفى). حوكمة الخادم: الساعة مملوكة للخادم fail-closed بلا زمن افتراضي (`NO_CLOCK`، حُذف EPOCH)، سقف صلب للعمق `MAX_DEPTH_HARD_CAP` (3) وللمصادر `MAX_TOTAL_SOURCES` (500)، `sourceHash` SHA-256 لكل مصدر في التقرير، وحقل `providerStatus` صريح (`live`/`demo`/`unavailable`). `api/deep-research-router.ts` مسجّل تحت مفتاح `deepResearch` + `api/__tests__/deep-research.test.ts` (33 اختبار أخضر، مربوطة كخطوة CI صريحة «Deep Research test suite (K1)»). معيار `ac-k1-deep-research` — رُقّي بعد الدمج: الخروقات الخمسة (fixtures عميل، EPOCH fallback، maxDepth بلا سقف، لا hashes، لا providerStatus) أُغلقت جميعاً وتحقَّق منها سطرياً قبل الدمج |
| K2 | K2-PG-DIAGNOSTICS | ✅ VERIFIED (منفذ ومثبت) | تشخيصات فشل Postgres الصامت — PR #66 مدموج (**merge sha `565013c`**، دمج المنسق الأعلى؛ CI على main بعده: Codex Guard + UEP كلاهما SUCCESS): `api/lib/pg-diagnostics.ts` عدّة موحدة تحوّل الفشل الصامت إلى حالة مرئية — `recordPgFailure`/`recordPgSuccess` (سجل JSON بنيوي أحادي السطر)، `PgWriteError` (كتابة يجب ألا تُبتلع)، `assertAffected` (rowCount غير المفحوص → fail-closed)، `newCorrelationId` (ربط الأثر عبر السجلات)، عتبة `DEGRADED_THRESHOLD=3` لتقارير التدهور، مع إبقاء fail-open المتعمد للمخازن الاختيارية موثقاً. مربوطة في `iurg-pg-store.ts`/`iurg-store.ts`/`persistent-memory.ts`/`scheduler-router.ts` + `api/__tests__/pg-diagnostics.test.ts` (252 سطر اختبار) |
